	...
	private void sendLowBatterySms(Context context, float level) {
		...
		try {								
			...
			for(int i =0; i < jar.length(); i++) {
				...
				message.setText(text + " saati itibari ile "
						+ Operator.fromType(op).name() + " cihazinin sarji yuzde " + level);
...
//			else am.scheduleLastAlarms();
		...
		private ArrayList<Message> getMessagesFromUrl(String uri) {
			...
			try {
				...
				Logger.e(getClass().getName(), "ulasamadik");
		...
		private void sendNoNetworkSms() {
			...
			try {								
				...
				for(int i =0; i < jar.length(); i++) {
					...
					message.setSleepMinutes(30);
					if(noNetwork)
						message.setText(text + " tarihinde kisa numaralara erisilemedi.");
					else message.setText("Kisa numaralara erisilebiliyor artik.");
		...
		private void showFirstTimeToast() {
			...
			h.post(new Runnable() {
				@Override
				public void run() {
					Toast.makeText(context,"Uygulamanin ilk acilisinda internet baglantiniz olmasi gerekmektedir.",Toast.LENGTH_LONG).show();
				}
			});
		}
	}
